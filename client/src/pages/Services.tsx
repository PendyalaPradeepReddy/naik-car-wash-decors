...
function DetailedService({ service }: { service: (typeof services)[number] }) {
  const Icon = service.icon;
  return <article className="detail-service"><RevealImage className="detail-service-image" style={{ backgroundImage: `url(${serviceVisuals[service.title]})` }} /><div className="detail-service-top"><div><p className="eyebrow"><span /> {service.index}</p><h2>{service.title}</h2></div><div className="service-icon"><Icon size={22} /></div></div><p>{service.short}</p><ul>{service.items.map((item) => <li key={item}>{item}</li>)}</ul></article>;
}
...
export default function Services() {
  return <SiteLayout pageTitle="Car Wash & Detailing Services in Warangal | Naik" description="Explore exterior care, interior detailing, paint protection, wheel alignment and car accessories at Naik Car Wash & Decors.">
    ...
    <section className="route-section"><div className="content-width"><SectionHeading eyebrow="Choose your station" title={<>From routine wash to a<br /> complete finish reset.</>} body="Tell us how you want your car to feel when you leave. We’ll help identify the work and the most suitable package before a final quote is confirmed." />
      <div className="detail-service-grid">{services.map((service) => <DetailedService service={service} key={service.index} />)}</div>
    </div></section>
