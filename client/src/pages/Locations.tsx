...
export default function Locations() {
  ...
  return <SiteLayout pageTitle="Car Wash Locations in Hanamkonda | Naik" description="Visit Naik Car Wash & Decors at KUC X Road or Fatima Junction in Hanamkonda, Warangal.">
    ...
    <section className="route-section"><div className="content-width"><SectionHeading eyebrow="Choose a branch" title={<>Pick the location that<br /> works for today.</>} body="Use the selector to see the full address, call a branch, send a WhatsApp message or get driving directions." />
      ...
      <div className="map-link-panel"><div><p className="eyebrow"><span /> Google Maps</p><h3>Open a branch in Maps.</h3><p>Use a direct Google Maps link for the selected Naik Car Wash & Decors location. No embedded map is loaded on this page.</p></div><div className="map-link-actions">{branches.map((item) => <a key={item.id} href={item.directions} target="_blank" rel="noreferrer" className="btn btn--dark"><MapPin size={15} /> {item.name} map <ArrowUpRight size={14} /></a>)}</div></div>
    </div></section>
